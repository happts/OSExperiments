...
//    }
    ...
    public int[] getNotfreeZone(){
        LinkedList<Integer> notFreeIDs = new LinkedList<>();
        int i ;
        for(i=0;i<zones.size();i++){
            Zone tmp = zones.get(i);
            if(!tmp.isFree){
                for (int j = 0; j <tmp.size;j++){
                    notFreeIDs.add(j+tmp.head);
                }
            }
        }
        Integer[] integers =  notFreeIDs.toArray(new Integer[0]);
        return Arrays.stream(integers).mapToInt(Integer::valueOf).toArray();
    }

    public int[] getZonesID(){
        int[] result = new int[zones.size()];
        for(int i=0;i<result.length;i++){
            result[i] = zones.get(i).head;
        }
        return result;
    }
}